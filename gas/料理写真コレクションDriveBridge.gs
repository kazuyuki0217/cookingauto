...
const RYORI_PHOTO_STATE_KEY = 'RYORI_PHOTO_LAST_SIGNATURE';
...
function 料理写真コレクションCheck() {
  ...
  // 同じファイルIDでも中身が更新された場合は必ず再取り込みする。
  const signature = [
    latest.getId(),
...
function uploadRyoriPhotoToGitHub_(content, originalName) {
  ...
  const payload = {
    message:'料理写真コレクション: 写真を受信 '+originalName,
    content:content,
    branch:branch
  };
  ...
  if (put.getResponseCode() !== 200 && put.getResponseCode() !== 201) {
    throw new Error('GitHub転送失敗 HTTP '+put.getResponseCode()+' '+put.getContentText());
  }
}
