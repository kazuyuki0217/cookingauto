/* Pointカズ: Google Drive受信箱 -> GitHub incoming/latest_photo.b64 */
const POINT_KAZU_DRIVE_FOLDER = 'Pointカズ受信箱';
const POINT_KAZU_GITHUB_PATH = 'incoming/latest_photo.b64';
const POINT_KAZU_STATE_KEY = 'POINT_KAZU_LAST_FILE_ID';

function PointKazuDriveSetup() {
  const folder = getOrCreatePointKazuDriveFolder_();
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === 'PointKazuDriveCheck') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('PointKazuDriveCheck').timeBased().everyMinutes(5).create();
  console.log('受信箱: ' + folder.getUrl());
...
function PointKazuDriveCheck() {
  ...
  if (latest.getId() === props.getProperty(POINT_KAZU_STATE_KEY)) return 'ALREADY_PROCESSED';
  const encoded = Utilities.base64Encode(latest.getBlob().getBytes());
  uploadPointKazuPhotoToGitHub_(encoded, latest.getName());
...
function getOrCreatePointKazuDriveFolder_() {
  const props = PropertiesService.getScriptProperties();
  const id = props.getProperty('POINT_KAZU_DRIVE_FOLDER_ID');
  if (id) { try { return DriveApp.getFolderById(id); } catch(e) {} }
  const it = DriveApp.getFoldersByName(POINT_KAZU_DRIVE_FOLDER);
  const folder = it.hasNext() ? it.next() : DriveApp.createFolder(POINT_KAZU_DRIVE_FOLDER);
  props.setProperty('POINT_KAZU_DRIVE_FOLDER_ID', folder.getId());
  return folder;
}

function uploadPointKazuPhotoToGitHub_(content, originalName) {
  ...
  const url = 'https://api.github.com/repos/' + encodeURIComponent(owner) + '/' + encodeURIComponent(repo) + '/contents/' + POINT_KAZU_GITHUB_PATH;
  ...
  const payload = {message:'Pointカズ: Drive料理写真を受信 '+originalName, content:content, branch:branch};
